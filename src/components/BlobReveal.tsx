...
export default function BlobReveal({
    ...
    startAlign = "bottom",
    replay = true,
    transition = { type: "tween", duration: 2, delay: 0.03, ease: "easeOut" } as unknown as BlobRevealTransition,
    style,
}: BlobRevealProps) {
    const trDuration =
        typeof transition?.duration === "number" ? transition.duration : 2;
    const trDelay =
        typeof transition?.delay === "number" ? transition.delay : 0.03;
    ...
    const appearDelayRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    useEffect(() => {
        ...
        if (inView && progress < 0.99) {
            if (scrollTweenRef.current || scrollDelayRef.current) return;
    ...
    useEffect(() => {
        return () => {
            ...
            if (appearDelayRef.current) {
                clearTimeout(appearDelayRef.current);
                appearDelayRef.current = null;
            }
            if (scrollDelayRef.current) {
                clearTimeout(scrollDelayRef.current);
                scrollDelayRef.current = null;
    ...
    useEffect(() => {
        if (!hasImage || appeared || !inView) return;
        appearDelayRef.current = setTimeout(() => {
            appearDelayRef.current = null;
            setAppeared(true);
        }, trDelay * 1000);
        return () => {
    ...
    }, [hasImage, appeared, inView, trDelay]);
